...
// Force Gradle to use the correct Node.js binary from NODE_BINARY property
project.ext.react = [
    nodeExecutableAndArgs: [project.hasProperty("NODE_BINARY") ? project.property("NODE_BINARY") : "node"]
]
...
// 👇 ADD THIS AT THE VERY END — fix for NODE_BINARY issue
def nodeBinary = project.hasProperty("NODE_BINARY") ? project.getProperty("NODE_BINARY") : "node"
tasks.matching { it.name == "generateCodegenSchemaFromJavaScript" }.configureEach {
  doFirst {
    println("[VisionCamera] Using node binary: $nodeBinary")
